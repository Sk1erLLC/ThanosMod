...
public class ThanosModGui extends GuiScreen {
    ...
    public void initGui() {
        ...
        reg(new GuiButton(++id, width / 2 - 100, 3, "TOGGLE"), guiButton -> {
            StringBuilder append = new StringBuilder().append(TextFormatting.YELLOW).append("Mod Status: ");
            if (instance.enabled)
                append.append(TextFormatting.GREEN).append("Enabled");
            else append.append(TextFormatting.RED).append("Disabled");
        ...
        reg(new GuiButton(++id, width / 2 - 100, 25, "MODE"), guiButton -> {
            StringBuilder append = new StringBuilder().append(TextFormatting.YELLOW).append("Mode: ");
            int mode = instance.MODE;
            append.append(TextFormatting.AQUA);
            if (mode == 0) {
        ...
        regSlider(new GuiSlider(++id, width / 2 - 100, 47, 200, 20, TextFormatting.YELLOW + "Dust Render Distance: " + TextFormatting.AQUA, "", 1, 64, instance.RENDER_DISTANCE, false, true, slider -> {
            instance.RENDER_DISTANCE = slider.getValueInt();
        }));
        regSlider(new GuiSlider(++id, width / 2 - 100, 69, 200, 20, TextFormatting.YELLOW + "Animation Start Distance: " + TextFormatting.AQUA, "", 1, 64, instance.DISTANCE, false, true, slider -> {
            instance.DISTANCE = slider.getValueInt();
        }));
        reg(new GuiButton(++id, width / 2 - 100, 69 + 22, "BLENDING"), guiButton -> {
            StringBuilder append = new StringBuilder().append(TextFormatting.YELLOW).append("Blending: ");
            append.append(TextFormatting.AQUA);
        ...
        regSlider(new GuiSlider(++id, width / 2 - 100, 69 + 44, 200, 20, TextFormatting.YELLOW + "Speed: " + TextFormatting.AQUA, "", .5D, 2D, instance.speed, true, true, slider -> {
